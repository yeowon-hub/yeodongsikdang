...
import { Refrigerator, BookOpen, Package, Snowflake } from 'lucide-react'
...
const tabs = [
  ...
  { to: '/recipes', label: '레시피', icon: BookOpen, match: '/recipes' },
] as const
...
export function BottomTabBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white safe-bottom shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
      <div className="mx-auto flex max-w-lg items-stretch justify-around px-1 pt-1.5 pb-1.5">
        {tabs.map(({ to, label, icon: Icon, match }) => {
          const isActive = location.pathname.startsWith(match)
          const showBadge = match.startsWith('/fridge') && expiringCount !== undefined && expiringCount > 0

          return (
            <NavLink
              key={to}
              to={to}
              className={`relative flex min-w-0 flex-1 flex-col items-center gap-0.5 rounded-lg px-1 py-1.5 text-[10px] font-medium transition-colors sm:text-xs ${
