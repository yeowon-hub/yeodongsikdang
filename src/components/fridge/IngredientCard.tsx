...
export function IngredientCard({ ingredient, onClick, compact, mini }: IngredientCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group relative flex shrink-0 flex-col items-start rounded-xl border bg-white text-left shadow-sm transition-all hover:shadow-md active:scale-[0.98] ${
    ...
    >
      <span
        className={`truncate font-medium text-gray-800 ${mini ? 'w-full text-[10px] leading-tight' : ''}`}
      >
        {ingredient.name}
      </span>
      <span className={`text-gray-500 ${mini ? 'text-[9px] leading-tight' : 'mt-0.5 text-xs'}`}>
        {ingredient.quantity}
        {ingredient.unit}
      </span>
